...
def multiply_factors(Factors):
    ...
    # IMPLEMENT
    def recursive_multiply(combined_factor, factors, new_scope):
        # Base case，calculate the multiplication
        if len(new_scope) == 0:
            product = 1
            for f in factors:
                product *= f.get_value_at_current_assignments()
            combined_factor.add_value_at_current_assignment(product)

        # Otherwise, loop through all value assignment
        else:
            for value in new_scope[0].domain():
                new_scope[0].set_assignment(value)
                recursive_multiply(combined_factor, factors, new_scope[1:])
    # Initialize the new factor
    name = ''
    scope = []
    for factor in Factors:
        # Get the name and scope from given factors to form the new one
        name += factor.name
    ...
    recursive_multiply(result, Factors, scope)
    return result


# TODO: Check the change of the given factor
def restrict_factor(f, var, value):
    ...
    # IMPLEMENT
    def recursive_restrict_factor(org_factor, org_scope, new_values):
        # Base case，calculate the multiplication
        if len(org_scope) == 0:
            new_values.append(org_factor.get_value_at_current_assignments())
        else:
            for val in org_scope[0].domain():
                org_scope[0].set_assignment(val)
                recursive_restrict_factor(org_factor, org_scope[1:], new_values)
    # Initialize the new factor
    name = f.name + "_restrict_" + var.name + "_" + value
    scope = f.get_scope()
    scope.remove(var)
    factor = Factor(name, scope)
    # Use a list to temporarily store all the values
    values = []
    # Set the given value to given variable
    var.set_assignment(value)
    # Recursively restrict the factors
    recursive_restrict_factor(f, scope, values)
    # Set the values of the factor to be the one that stores all the values
...
def sum_out_variable(f, var):
    ...
    # IMPLEMENT
    def recursive_sum_out_variables(new_factor, org_factor, new_scope, variable):
        # Base case, get the summation of all possible values for assignment
        if len(new_scope) == 0:
            total = 0
            for value in variable.domain():
                variable.set_assignment(value)
                total += org_factor.get_value_at_current_assignments()
        ...
        # Otherwise, loop through all value assignment
            ...
            for value in new_scope[0].domain():
                new_scope[0].set_assignment(value)
                recursive_sum_out_variables(new_factor, org_factor, new_scope[1:], variable)
    ...
    scope.remove(var)
    factor = Factor(name, scope)
    # Recursively sum up the factors
    recursive_sum_out_variables(factor, f, scope, var)
    return factor
